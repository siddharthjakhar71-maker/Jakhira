...
import { ShieldCheck, UserCircle2, Settings2, Upload, Trash2, Plus, Pencil } from "lucide-react";

type SettingsTab = "profile" | "access-control" | "preferences";
...
export default function Settings() {
  ...
  const initialTab = (params.get("tab") as SettingsTab | null) || "profile";
  const [activeTab, setActiveTab] = useState<SettingsTab>(initialTab);
  const { userProfile, updateUserProfile, changePassword, logout, systemSettings } = useStore();
  const { canView, canEdit, canDelete, canApprove, isAdmin } = usePermissions();
  ...
  useEffect(() => {
    const next = params.get("tab") as SettingsTab | null;
    ...
    <AppLayout>
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Manage your personal account, access control, and ERP preferences in one compact workspace.</p>
        </div>

        <Tabs value={activeTab} onValueChange={changeTab} className="space-y-4">
          <TabsList className="grid w-full max-w-2xl grid-cols-3">
            <TabsTrigger value="profile" className="gap-2"><UserCircle2 className="h-4 w-4" />Profile</TabsTrigger>
            <TabsTrigger value="access-control" disabled={!isAdmin || !canView("Users")} className="gap-2"><ShieldCheck className="h-4 w-4" />Access Control</TabsTrigger>
            <TabsTrigger value="preferences" className="gap-2"><Settings2 className="h-4 w-4" />System / Preferences</TabsTrigger>
          </TabsList>
            ...
            <div className="grid gap-4 xl:grid-cols-[.8fr_1.2fr]">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Backup controls remain available under Settings.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
